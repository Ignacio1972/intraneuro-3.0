    ...
    try {
        ...
        if (typeof apiRequest === 'function') {
            try {
                ...
                // Reconstruir el chat desde los registros individuales
                const historiaMessages = [];
                const tareasMessages = [];

                // Convertir cada observación en un mensaje de chat
                if (Array.isArray(obsResponse)) {
                    obsResponse.forEach(obs => {
                        if (obs.observation && typeof obs.observation === 'string' && obs.observation.trim()) {
                            // Verificar si es JSON o texto simple
                            try {
                                const parsed = JSON.parse(obs.observation);
                                if (Array.isArray(parsed)) {
                                    // Es un array de mensajes antiguos, procesarlos
                                    parsed.forEach(msg => {
                                        if (msg.text) {
                                            historiaMessages.push({
                                                id: msg.id || Date.now() + Math.random(),
                                                text: msg.text,
                                                timestamp: msg.timestamp || new Date(obs.created_at).toLocaleString('es-CL'),
                                                author: msg.author || obs.created_by || 'Sistema',
                                                type: 'received'
                                            });
                                        }
                                    });
                                ...
                                // Es texto simple, lo agregamos como mensaje
                                historiaMessages.push({
                                    id: obs.id,
                ...
                // Convertir cada tarea en un mensaje de chat
                                    ...
                                    parsed.forEach(msg => {
                                        if (msg.text) {
                                            tareasMessages.push({
                                                id: msg.id || Date.now() + Math.random(),
                                                text: msg.text,
                                                timestamp: msg.timestamp || new Date(task.created_at).toLocaleString('es-CL'),
                                                author: msg.author || task.created_by || 'Sistema',
                                                type: 'received'
                                            });
                                        }
                                    });
                ...
                chatNotesData[patientId] = {
                    historia: historiaMessages,
                    pendientes: tareasMessages
                };
            } catch (apiError) {
                console.error('Error crítico cargando del servidor:', apiError);
            ...
            // Si no hay API disponible, inicializar vacío
            chatNotesData[patientId] = {
                historia: [],
                pendientes: []
            };
...
async function sendChatNote(patientId) {
    ...
    // Guardar SOLO el mensaje nuevo en el backend
    try {
        if (currentChatTab === 'historia') {
            // Guardar solo el texto del mensaje nuevo
            if (typeof PacientesAPI !== 'undefined' && PacientesAPI.saveObservationsAPI) {
                await PacientesAPI.saveObservationsAPI(patientId, newMessage.text);
            } else {
                // Fallback directo al API
                await apiRequest(`/patients/${patientId}/admission/observations`, {
                    method: 'POST',
                    body: JSON.stringify({
                        observation: newMessage.text,
                        created_by: currentUser || 'Usuario'
                    })
                });
            }
        } else {
            // Guardar solo el texto de la tarea nueva
            if (typeof PacientesAPI !== 'undefined' && PacientesAPI.savePendingTasksAPI) {
                await PacientesAPI.savePendingTasksAPI(patientId, newMessage.text);
            } else {
                // Fallback directo al API
                await apiRequest(`/patients/${patientId}/admission/tasks`, {
                    method: 'POST',
                    body: JSON.stringify({
                        task: newMessage.text,
                        created_by: currentUser || 'Usuario'
                    })
                });
            }
...
async function saveChatNotes(patientId) {
    try {
        ...
        // Obtener datos y verificar si hay contenido real
        const historiaData = chatNotesData[patientId]?.historia || [];
        const pendientesData = chatNotesData[patientId]?.pendientes || [];

        // Solo procesar si hay datos para guardar
        if (historiaData.length === 0 && pendientesData.length === 0) {
